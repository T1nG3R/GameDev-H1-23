...
namespace Core.Movement.Controller
{
    public class DirectionalMover
    {
        ...
        public void MoveVertically(float direction)
        {
            ...
            Vector2 velocity = _rigidbody.velocity;
            velocity.y = direction * _statValueGiver.GetStatValue(StatType.Speed)/4;
            _rigidbody.velocity = velocity;

            if (direction == 0)
                return;

            float verticalPosition = Mathf.Clamp(_rigidbody.position.y, _directionalMovementData.MinVerticalPosition,
        ...
        private void UpdateSize()
        {
            ...
            float currentSizeModificator = _directionalMovementData.MinSize +
